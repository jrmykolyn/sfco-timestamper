 ...
 * @return {Promise}
 */
/// TODO[@jrmykolyn] - Consider renaming function to `run`.
function init( args ) {
	return literati.read( `${os.homedir()}/${GLOBAL_CONFIG}` )
		...
		.then( ( pathStr ) => {
			return literati.read( pathStr )
				.then(
					( data ) => {
						if ( !decoder.write( data ).includes( getTodayAsTimestamp() ) || args.includes( '--force' ) ) {
							return pathStr;
						} else {
		...
		.then( ( pathStr ) => {
			return new Promise( ( resolve, reject ) => {
				fs.appendFile( pathStr, getDumpMsg( args.includes( '--force' ) ), 'utf8', ( err ) => {
					if ( err ) {
						reject( err );
					} else {
						resolve( pathStr );
...
/**
 * Given a `type` and `key`, function returns the corresponding message string from the `MESSAGES` object.
 *
