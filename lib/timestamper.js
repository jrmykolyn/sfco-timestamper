	...
	return new Promise( ( resolve ) => {
		...
		.then( ( pathStr ) => {
			return literati.read( pathStr )
				.then(
					( data ) => {
						if ( !decoder.write( data ).includes( getTodayAsTimestamp() ) || options.force ) {
							return pathStr;
						} else {
		...
		.then( ( pathStr ) => {
			return new Promise( ( resolve, reject ) => {
				fs.appendFile( pathStr, getDumpMsg( options.force ), 'utf8', ( err ) => {
					if ( err ) {
						reject( err );
					} else {
						resolve( pathStr );
		...
		.then( ( pathStr ) => {
			return new Promise( ( resolve, reject ) => {
				...
				var commitMsg = getCommitMsg( options.force );
